function deleteForwardSermonInfoTrigger() {
  let remove_array = []
  ...
  for (var i = 0; i < oldTrigger.length; i++) {
    ...
    if (ScriptApp.getScriptTriggers()[i].getHandlerFunction() == "forwardSermonInfo") {
      logMessage(getCallStackTrace() + `: Found \"${ScriptApp.getScriptTriggers()[i].getHandlerFunction()}\" trigger !!! `);
  ...
  remove_array.forEach(function (row) {
    ...
    logMessage(getCallStackTrace() + ": " + Utilities.formatDate(new Date(), Session.getScriptTimeZone(), 'yyyy-MM-dd-HH:mm:ss') + ': Deleting forwardSermonInfo !!!');
